#!/usr/bin/env python3
"""Extract rendered Devbox docs from a Mintlify preview server."""
...
SITE_URL = "https://www.jetify.com"
SOURCE_REPOSITORY = "https://github.com/jetify-com/docs"
PRODUCT = "Devbox"
DOC_ROOT = "docs/devbox"
...
def nav_doc_ids(source: Path) -> list[str]:
    ...
    products = docs_config.get("navigation", {}).get("products", [])
    for product in products:
        if product.get("product") != PRODUCT:
            continue
        pages = list(iter_pages(product.get("pages", [])))
        for group in product.get("groups", []):
            pages.extend(iter_pages(group.get("pages", [])))
        return list(unique(pages))
    raise RuntimeError(f"No {PRODUCT!r} product found in docs.json navigation")
...
def start_preview(source: Path) -> tuple[subprocess.Popen[str], str]:
    process = subprocess.Popen(
        ["pnpm", "exec", "mint", "dev", "--no-open"],
        cwd=source,
...
def page_title(soup: BeautifulSoup) -> str:
    ...
    if title is not None:
        return title.get_text(" ", strip=True).removesuffix(" - Jetify Docs")
...
def section_html(page: Page, rendered_html: str, source: Path) -> str:
    ...
    if content is None:
        raise RuntimeError(f"No #content element found for {page.route}")
...
def main() -> None:
    ...
    process, base_url = start_preview(source)
    sections = []
    try:
    ...
    args.out.write_text(
        "\n".join(
            [
                ...
                "<body>",
                f"<h1>{TITLE}</h1>",
                f'<p>Source site: <a href="{SITE_URL}/docs/devbox">{SITE_URL}/docs/devbox</a></p>',
                f'<p>Source repository: <a href="{SOURCE_REPOSITORY}">jetify-com/docs</a></p>',
                *sections,
                "</body>",
                "</html>",
                "",
